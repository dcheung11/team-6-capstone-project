...
Table \ref{tab:usabilitySurvey} summarizes the responses of \hyperref[MIN_TESTERS]{\texttt{MIN\_TESTERS}} participants (Response IDs 0--4) to our usability questionnaire. The particiapants had no prior experience with the product. Each row corresponds to one survey question.
...
& \textit{(none)}
& \parbox[t]{0.11\textwidth}{\raggedright Invalid email accepted.  Misspelled \texttt{@gmail.com} and it still allowed it.}
& \textit{(none)}
...
& 4
& \textit{(none)}
& \parbox[t]{0.11\textwidth}{\raggedright i didn't use them}
& \parbox[t]{0.11\textwidth}{\raggedright NA}
& \textit{(none)} \\ \hline
