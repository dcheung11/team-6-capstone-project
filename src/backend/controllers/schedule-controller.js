...
const generateSchedule = async (req, res) => {
  try {
    ...
    res.json({ message: "Schedule generated successfully", schedule: schedule});
    console.log("Fully generated schedule: \n");
    //print every game in the schedule
    for (let i = 0; i < schedule.games.length; i++) {
        let game = await Game.findById(schedule.games[i]).populate('date time field');
        console.log(game)
    }
  } catch (error) {
...
const generateGameSlots = async (schedule, startDate, endDate) => {
    ...
    while (currentDate <= end) {
        if (currentDate.getDay() >= 1 && currentDate.getDay() <= 5) { // Weekdays
    ...
    for (const [team1, team2] of pairings) {
        for (const slot of availableSlots) {
            ...
            if (!t1Busy && !t2Busy) {
                ...
                teamAvailability.set(team1._id, (teamAvailability.get(team1._id) || new Set()).add(slotDate));
                teamAvailability.set(team2._id, (teamAvailability.get(team2._id) || new Set()).add(slotDate));
                break;
            }
        }
    }
